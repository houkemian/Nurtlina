...
Uses a synchronous DB connection (DATABASE_SYNC_URL) so Alembic can run
migrations without async complexity. The app runtime uses the async
...
# Import all models so autogenerate can diff them against the DB schema
